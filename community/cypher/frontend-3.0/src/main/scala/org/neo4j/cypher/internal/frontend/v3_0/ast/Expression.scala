...
import org.neo4j.cypher.internal.frontend.v3_0.{SemanticState, _}
...
trait FunctionTyping { self: Expression =>
  ...
  def semanticCheck(ctx: ast.Expression.SemanticContext): SemanticCheck = {
    val specifyTyper: ( => TypeSpec) => (SemanticState) => Either[SemanticError, SemanticState] = specifyType _
    arguments.semanticCheck(ctx) chain
      TypeChecker(signatures, arguments, specifyTyper)
  }
}
