...
import org.neo4j.cypher.internal.compiler.v2_2.executionplan.{Effects, WritesProperty}
...
case class DeletePropertyAction(element: Expression, propertyKey: KeyToken)
  ...
  def localEffects(symbols: SymbolTable) = Effects(WritesProperty(propertyKey.name))
}
