...
//    result should use("RepeatableRead", "Eager")
  ...
  test("on match set label on unstable iterator should be eager") {
    ...
    println(updateWithBothPlanners(s"EXPLAIN $query").executionPlanDescription())
  ...
  test("THIS TEST SHOULD BE GREEN -- also other combinations where MERGE replaces MATCH and CREATE") {
    ...
    //val query = "UNWIND range(0, 1) AS i MATCH (x) WITH * MERGE (m {v: i % 2}) CREATE ({v: (i + 1) % 2}) RETURN count(*)"
...
//  test("AND THIS TEST SHOULD ALSO BE GREEN -- also other combinations where MERGE replaces MATCH and CREATE") {
//    createNode()
//    createNode()
//
//    //val query = "UNWIND range(0, 9) AS i MATCH (x) MERGE (m {v: i % 2}) ON CREATE SET m:Merged CREATE ({v: (i + 1) % 2}) RETURN count(*)"
//    val query = "UNWIND range(0, 1) AS i MATCH (x) WITH * MATCH (m {v: i % 2}) CREATE ({v: (i + 1) % 2}) RETURN count(*)"
//
//    val result = updateWithBothPlanners(query)
//    result.columnAs[Long]("count(*)").next shouldBe 20
//    assertStats(result, nodesCreated = 22, propertiesWritten = 22, labelsAdded = 2)
...
//  }
  ...
  // TODO: This produces a broken plan (looks like a join is planned without considering the update graph),
  // so the create is planned after the cartesian product of the two matches
  ignore("setting property in tail should be eager if overlap with unwind") {
    createNode()
    createNode()
    createNode("prop" -> 42)
    createNode("prop" -> 42)
    val query = "UNWIND range(0, 1) as i MATCH (n) CREATE (m) WITH * MATCH (o) SET n.prop = 42 RETURN count(*) as count"
  ...
  test("should be eager if removing a label that's matched by an unstable iterator") {
    createLabeledNode("A", "B")
    createLabeledNode("A", "B")
    val query = "MATCH (m:A), (n:B) REMOVE n:B RETURN count(*)"
  ...
  ignore("should be eager between conflicting read/write separated by UNWIND between reads and writes -- MERGE") {
    createNode()
    createNode()
...
//  test("multiple merge node patterns should work...") {
//    createNode()
//    createNode()
//
//    val query = "MERGE (n) MERGE (m) WITH * RETURN count(*)"
//
//    val result = updateWithBothPlanners(query)
//    result.columnAs[Long]("count(*)").next() should equal(4)
//    assertStats(result, nodesCreated = 4)
